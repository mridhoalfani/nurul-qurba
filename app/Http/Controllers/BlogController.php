...
class BlogController extends Controller
{
    ...
    public function readPost($slug)
    {
        if (!$slug) {
            ...
            //     ->where(function ($query) use ($post_tags) {
            ...
            $related_posts = Post::where('id', '!=', $post->id)
                ...
                ->orWhere(function ($query) use ($post) {
                    $query->where('post_title', 'like', $post->post_title)
                        ->where('id', '!=', $post->id);
                })
                ->inRandomOrder()
                ->limit(3)
            ...
            $related_posts = $related_posts->unique('id')->values();

            if ($related_posts->count() < 3) {
                $additional_posts = Post::where('id', '!=', $post->id)
                    ->inRandomOrder()
                    ->limit(3 - $related_posts->count())
                    ->get();

                $related_posts = $related_posts->merge($additional_posts)->unique('id')->values();
            }
